...
class Gridding(torch.nn.Module):
    ...
    def forward(self, ptcloud):
        ...
        ptcloud: (B, N, 3)
        """
        ptcloud = ptcloud * self.scale  # 点云并没有归一化到[-1, 1]
        _ptcloud = torch.split(ptcloud, 1, dim=0)
        grids = []
        for p in _ptcloud:
            non_zeros = torch.sum(p, dim=2).ne(0)  # & ???  x + y + z != 0  # (1, N)
            p = p[non_zeros].unsqueeze(dim=0)  # (1, n, 3)
            grids.append(GriddingFunction.apply(self.scale, p))

        return torch.cat(grids, dim=0).contiguous()  # (B, scale^3)
