...
from models.snowflakenet import SkipTransformer, SPD
...
class UpSample(nn.Module):
    def __init__(self, channel=128, ratio=1):
        ...
        self.sa3 = TransformerBlock(512, channel * ratio)  # ~ ablation: shuffle
        ...
        self.conv_ps = nn.Conv1d(channel * ratio, channel * ratio, kernel_size=1)  # ~ ablation: shuffle
    ...
    def forward(self, x, coarse, feat_g):
        ...
        # y3_ = self.ps(y3)  # B, 128, N x r  # & ablation: concat2
        y3 = self.conv_ps(y3).reshape(batch_size, -1, N * self.ratio)  # ~ ablation: shuffle
...
class Model(nn.Module):
    def __init__(self, num_pc=256, num_down=256, k=16, ratios=[4, 8]):
        ...
        self.spd1 = SPD(up_factor=ratios[0])
        self.spd2= SPD(up_factor=ratios[1])
    ...
    def forward(self, x):
        ...
        refine1, feat_fine1 = self.spd1(x_.transpose(1, 2).contiguous(), feat_g)
        refine2, feat_fine2 = self.spd2(refine1, feat_g, feat_fine1)

        return coarse, fusion, down_fusion, coarse_, x_, refine1.transpose(1, 2).contiguous(), refine2.transpose(1, 2).contiguous()
        # return coarse, fusion, x_, refine1.transpose(1, 2).contiguous(), refine2.transpose(1, 2).contiguous()
