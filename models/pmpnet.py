...
class ModelNoise(nn.Module):
    ...
    def train_step(self, in_cloud, gt_cloud, l2_cd, emd):
        pcds, deltas = self(in_cloud)
            
        cd1 = l2_cd(pcds[0], gt_cloud)
        cd2 = l2_cd(pcds[1], gt_cloud)
        cd3 = l2_cd(pcds[2], gt_cloud)
        loss_cd = cd1 + cd2 + cd3
        ...
        loss = loss_cd * 1000 + loss_pmd * 1e-2

        return {'cd1': cd1, 'cd2': cd2, 'cd3': cd3, 'loss_cd': loss_cd, 'loss_pmd': loss_pmd, 'loss': loss}

    def eval_step(self, in_cloud, gt_cloud, l2_cd):
        pcds, deltas = self(in_cloud)
        metric = l2_cd(pcds[2], gt_cloud)
        return metric, pcds[2]