...
from pointnet2_ops.pointnet2_utils import ball_query, three_nn, three_interpolate, grouping_operation
...
def sample_and_group(xyz, points, npoint, nsample, radius, use_xyz=True):
    ...
    new_xyz, _ = sample_farthest_points(xyz_flipped, K=npoint)  # (B, npoint, 3)

    idx = ball_query(radius, nsample, xyz_flipped, new_xyz) # (B, npoint, nsample)
    new_xyz = new_xyz.transpose(1, 2).contiguous()  # (B, 3, npoint)
...
def fps(pcd, n=2048, return_idx=False):
    """
    Args
        pcd: (b, 16384, 3)
    ...
    """
    new_pcd, idx = sample_farthest_points(pcd, K=n)
    if return_idx:
        return new_pcd, idx
    else:
        return new_pcd
